...
//存储Bean中@Resource和@Autowired注解的属性的Field和Method对象，String是Bean的Class名，InjectionMetadata封装了存储注入对象的Collection
	...
	public void postProcessMergedBeanDefinition(RootBeanDefinition beanDefinition, Class<?> beanType, String beanName) {
		if (beanType != null) {//Autowired成员变量
...
// 这里的InjectionMetadata 就是在前面发现的需要注入的元素的封装，在之前通过这个后置处理器设置过
	...
	private InjectionMetadata findAutowiringMetadata(String beanName, Class<?> clazz, PropertyValues pvs) {
		...
		if (InjectionMetadata.needsRefresh(metadata, clazz)) {//如果需要更新，把之前对@Autowired和@Resource的与解析操作重做一遍
...
//再遍历Method对象
	...
	private class AutowiredFieldElement extends InjectionMetadata.InjectedElement {
		...
		protected void inject(Object bean, String beanName, PropertyValues pvs) throws Throwable {
			...
			else {//DependencyDescriptor依赖的封装对象
