...
public abstract class AbstractBeanDefinitionReader implements EnvironmentCapable, BeanDefinitionReader {
	...
	public int loadBeanDefinitions(String location, Set<Resource> actualResources) throws BeanDefinitionStoreException {
		ResourceLoader resourceLoader = getResourceLoader();//取得ResourceLoader，使用的是DefaultResourceLoader
		if (resourceLoader == null) {
			throw new BeanDefinitionStoreException(
					"Cannot import bean definitions from location [" + location + "]: no ResourceLoader available");
		}
//	对resource路径模式进行解析，actualResources Resource集合，只想我们已经定义好的BeanDefinitions信息，可以是多个文件
		if (resourceLoader instanceof ResourcePatternResolver) {
			//
			try {//调用getResources完成具体Resource定位
				Resource[] resources = ((ResourcePatternResolver) resourceLoader).getResources(location);
				int loadCount = loadBeanDefinitions(resources);//!!载入XML形式的BeanDefinition
			...
			// 调用resourceLoader完成具体Resource定位
			Resource resource = resourceLoader.getResource(location);
			int loadCount = loadBeanDefinitions(resource);//取得Resource的具体过程
			if (actualResources != null) {
				actualResources.add(resource);
			}
			if (logger.isDebugEnabled()) {
