...
//正在装载的配置文件
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {//这里得到xml文件，并得到io的inputSource准备进行读取
	 ...
	 *///具体的读取过程可以在doLoadBeanDefinitions中找到，这是从特定的xml中载入BeanDefinition的地方
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {//取的xml的Document对象，这个解析过程是由documentLoader完成的
			Document doc = doLoadDocument(inputSource, resource);
			return registerBeanDefinitions(doc, resource);//这里是对BeanDefinetion解析的详细过程，这个过程会使用到Spring的Bean配置规则
		}
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();//得到BeanDefinitionDocumentReader对来自XML的Beandefinition进行解析
		int countBefore = getRegistry().getBeanDefinitionCount();
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));//具体解析过程在registerBeanDefinitions完成
		return getRegistry().getBeanDefinitionCount() - countBefore;//createReaderContext(resource),返回documentReader上下文对象，包含次BeanDefinitionReader，进而注入BeanFactory和applicationContext
	}
